...
public class RestDetailDispatcher {
	...
	public boolean dispatch(RestDetail craft, StoreRecord record) {
		try {
			SendResult<Integer, RestDetail> sendResult = kafkaTemplate
			                .sendDefault(Integer.valueOf(record.getKey()), craft).get();
			...
			if (record.getRestCrewMinOffset() == -1)
				...
				record.setRestCrewMaxOffset((int) (recordMetadata.offset()));
			log.info(metaRecord);
			return true;
		}
		catch (Exception e) {
