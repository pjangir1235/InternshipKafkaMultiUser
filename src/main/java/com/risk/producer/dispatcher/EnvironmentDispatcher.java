  ...
  public boolean dispatch(Environment env, StoreRecord rec) {
    try {
      SendResult<Integer, Environment> sendResult =
          kafkaTemplate.sendDefault(rec.getKey(), env).get();

      RecordMetadata recordMetadata = sendResult.getRecordMetadata();
      if (rec.getEnvMinOffset() == -1) rec.setEnvMinOffset((int) recordMetadata.offset());
      else rec.setEnvMaxOffset((int) recordMetadata.offset());
      String metaRecord =
          "{offset - "
